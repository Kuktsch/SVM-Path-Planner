...
class KernelSVM:
    ...
    def kernel(self, x1, x2):
        # x1: (N, D), x2: (M, D)
        # возвращает матрицу ядра размера N×M
    ...
    def fit(self, X, y, max_iter=500):
        ...
        # Sequential minimal optimization обучение
        num_changed = 0
        examine_all = True
    ...
    def _take_step(self, i1, i2, K, y, f_cache):
        ...
        if i1 != i2:
            ...
            if L < H:
                ...
                # Проверка валидности eta
                if eta < 0:
                    # Вычисление нового alpha2
                    a2 = alpha2 - y2 * (E1 - E2) / eta
                    a2 = np.clip(a2, L, H)
                    
                    # Проверка значимости изменения
                    if abs(a2 - alpha2) >= 1e-5:
                        # Вычисление нового alpha1
    ...
    def decision(self, XY: np.ndarray) -> np.ndarray:
        return self.decision_function(XY)
    ...
    def params_summary(self) -> dict:
        if hasattr(self, "alphas") and self.alphas is not None:
            num_sv = int(np.sum(self.alphas > 1e-6))
        else:
            num_sv = 0
